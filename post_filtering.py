...
annotated['DP'] = annotated['Samples'].str.split('DP:', 1).str[1].str.split('_').str[0].astype(int)
annotated['MRD'] = annotated['Samples'].str.split('MRD:', 1).str[1].str.split(';').str[0].astype(int)
...
#in SNP db > 0.1% AF
filteredvariants7 = annotated.loc[(annotated['Global AF'].fillna(0) > 0.001) | (annotated['Non-Fin Eur AF'].fillna(0) > 0.001)].copy()
filteredvariants7['FilterReason'] = "> 0.1% in GnoMAD3"
annotated = annotated.loc[(annotated['Global AF'].fillna(0) <= 0.001) | (annotated['Non-Fin Eur AF'].fillna(0) <= 0.001)] #Filter out SNPS with db VAF of > 1%
filtereddf.loc[len(filtereddf)] = ['> 0.1% in GnoMAD3', len(annotated)]  
...
annotated[['Chrom.1', 'Pos', 'Reference allele', 'Alternate allele', 'Samples']].to_csv(path + samplename + '/merged_vcfs_annotated-filtered.txt', sep= '\t', index = False, header= False) #Filtered merged variants
